...
import { Input, Textarea, Toggle } from './components/FormField'
...
function RatingStars({ value }) {
  ...
  const rating = Math.min(5, Math.max(1, parseInt(value) || 0))
  return <span className="text-yellow-400">{stars.map((s) => (s <= rating ? '★' : '☆'))}</span>
}
...
export default function TestimonialsManager() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true)
    try {
      const res = await apiCall(`testimonials.php?page=${page}&limit=20&search=${encodeURIComponent(search)}`)
  ...
  const openEdit = (row) => {
    setEditItem(row)
    setFormData({
      name: row.name || '', role: row.role || '', company: row.company || '', text: row.text || '',
      rating: row.rating ?? 5, project: row.project || '', avatar: row.avatar || '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const payload = { ...formData, rating: Math.min(5, Math.max(1, parseInt(formData.rating) || 5)), is_active: formData.is_active ? 1 : 0 }
      if (editItem) {
        const res = await apiCall(`testimonials.php?id=${editItem.id}`, { method: 'PUT', body: JSON.stringify(payload) })
        if (res.success) { setShowModal(false); fetchItems() }
      } else {
        const res = await apiCall('testimonials.php', { method: 'POST', body: JSON.stringify(payload) })
        if (res.success) { setShowModal(false); fetchItems() }
      }
    } catch (err) { console.error(err) }
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await apiCall(`testimonials.php?id=${id}`, { method: 'DELETE' })
  ...
  const columns = [
    ...
    { header: 'Proje', accessor: 'project' },
    { header: 'Puan', accessor: 'rating', render: (row) => <RatingStars value={row.rating} /> },
    {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Müşteri yorumlarınızı yönetin — satırları sürükleyerek sıralayabilirsiniz</p>
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Yorum Düzenle' : 'Yeni Yorum Ekle'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Ad Soyad" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Müşteri adı" />
            <Input label="Rol / Ünvan" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} placeholder="Örn: CEO, Proje Yöneticisi" />
            <Input label="Şirket" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })} placeholder="Şirket adı" />
            <Input label="Proje" value={formData.project} onChange={(e) => setFormData({ ...formData, project: e.target.value })} placeholder="Proje adı" />
            <Input label="Puan (1-5)" type="number" min={1} max={5} value={formData.rating} onChange={(e) => setFormData({ ...formData, rating: parseInt(e.target.value) || 5 })} />
          ...
          <Textarea label="Yorum Metni" required value={formData.text} onChange={(e) => setFormData({ ...formData, text: e.target.value })} placeholder="Müşteri yorumu..." rows={4} />
